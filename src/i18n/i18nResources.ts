export const i18nResources = {
  en: {
    translation: {
      loginFieldName: "Name",
      loginFieldEmail: "E-mail",
      loginButtonSignIn: "Sign in",
      searchButtonMatch: "Fetch my Match!",
      searchButtonMatchNarrow: "Match",
      searchButtonSortBy: "Sort by",
      searchButtonSortByNarrow: "By",
      searchButtonAllBreeds: "All",
      searchMenuItemSignOut: "Sign out",
      searchMenuTitleFilterBreeds: "Include Breeds",
      searchMenuTitleSortDir: "Sort Results",
      searchMenuTitleSortBy: "Sort Results By",
      searchMenuItemSortDirAsc: "Ascending",
      searchMenuItemSortDirDesc: "Descending",
      searchMatchDialogBanner: "It's a Match!",
    },
  },
};
